...
def conv_forward_pass(image, filt, bias):
    ...
    # print(image.shape, filt.shape, out.shape)
    for pp in range(n_f):
        for i in range(out_dim):
            for j in range(out_dim):
                oPixelPos = i*out_dim + j+ pp*out_dim*out_dim
                out[oPixelPos] = 0
                for h in range(dp):
                    for k in range(f):
                        for l in range(f):
                            iPixelPos = ((i+k)*img_dim + (j+l)) + img_dim*img_dim*h
                            filtPos = (k*f+l) + f*f*h + f*f*dp*pp;
                            out[oPixelPos] += image[iPixelPos] * filt[filtPos] + bias[pp];
    out = np.reshape(out,(n_f,out_dim,out_dim))
    return out
