...
def conv_backward_pass(dconv_prev, conv_in, filt, flag):
    ...
    # print("p",padded_dconv.shape)
        featMat = []
        kernelMat = []

        for j in range(orig_dim):
            for k in range(orig_dim):
                rowmat = []
                for i in range(depth_f):
                    currmat = padded_dconv[i,j:j+fsize,k:k+fsize]
                    currmat = currmat.flatten()
                    # print(len(currmat))
                    rowmat.extend(currmat.tolist())
                # print(len(rowmat))
                featMat.append(rowmat)
        featMat = np.array(featMat)
    # print("f",featMat.shape)

        for j in range(depth_f):
            for k in range(fsize):
                for l in range(fsize):
                    rowmat = []
                    for i in range(num_filters):
                        rowmat.append(tmpf[i,j,k,l])
                    kernelMat.append(rowmat)
        kernelMat = np.array(kernelMat)
        # print("k",kernelMat.shape)
        output = np.matmul(featMat, kernelMat).T
        output = output.reshape((8,orig_dim,orig_dim))
